...
# log information
  logging.basicConfig(filename=plate_path_result+'/logfile.log', level=logging.INFO, format='%(asctime)s, %(levelname)s, %(filename)s, %(message)s')
...
#   plate_path = os.path.join(input_path,plate)
  ...
  #   logging.info('{}, find radius: success'.format(plate))
  # except:
  #   logging.error('{}, find radius: failed'.format(plate))
  ...
  foci_count_result.to_csv(os.path.join(plate_path_result,'foci_count_result.csv'))
    ...
    # logging.info('{}, detection process: complete'.format(plate))

  # except:
  #   logging.error('{}, detection process: failed'.format(plate))
